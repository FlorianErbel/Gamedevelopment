...
function PlatformManager:add(x, y, w, is_ground)
    add(self.list, {
        ...
        dead = false,                     -- für BreakablePlatform später
...
function PlatformManager:check_landing(player, prev_y)
    ...
    local foot_prev = prev_y + player.height
    local foot_now  = player.y + player.height

    for p in all(self.list) do
        if player.x + player.width > p.x and player.x < p.x + p.w then
